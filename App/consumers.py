...
def Obtener_conversacion(id):
    ...
    for mensaje in Mensajes.objects.all():
        if (
            str(mensaje.FROM_usuario.id) == str(amigo_1.id)
            and str(mensaje.TO_usuario) == str(amigo_2.id)
        ) or (
            str(mensaje.FROM_usuario.id) == str(amigo_2.id)
            and str(mensaje.TO_usuario) == str(amigo_1.id)
        ):
            messages.append(mensaje)
            print(mensaje)
            mensajes += (
                str(mensaje.FROM_usuario.usuario.username)
                + " :"
                + str(mensaje.Mensaje)
                + "\n"
            )
        ...
        if not user.is_anonymous:
            ...
            if int(user.usuario.id) in [int(visitado), int(id_usuario)]:
                ...
                Mensajes.objects.create(
                    FROM_usuario=user.usuario, TO_usuario=visitado, Mensaje=message
                )
                ...
                async_to_sync(self.channel_layer.group_send)(
                    ...
                    {
                        "type": "chat_message",
                        "message": {"message": message, "id_usuario": id_usuario},
